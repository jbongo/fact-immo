                            ...
                            <table id="example4"
                                ...
                                <tbody>
                                    @foreach ($compromisEncaissee as $compromi_enc)
                                        <tr>
                                            <td>
                                                @if ($compromi_enc->type_affaire == 'Vente')
                                            ...
                                            @if (Auth()->user()->role == 'admin')
                                                <td>
                                                    <strong> <a
                                                            href="{{ route('switch_user', Crypt::encrypt($compromi_enc->user->id)) }}"
                                                            data-toggle="tooltip"
                                                            title="@lang('Se connecter en tant que ') {{ $compromi_enc->user->nom }}">{{ $compromi_enc->user->nom }}
                                                            {{ $compromi_enc->user->prenom }}<i style="font-size: 17px"
                                            ...
                                            <td width="">
                                                @if ($compromi_enc->getFactureStylimmo() != null)
                                                    <a class="color-info" title="Télécharger la facture stylimmo"
                                                        href="{{ route('facture.telecharger_pdf_facture_stylimmo', Crypt::encrypt($compromi_enc->id)) }}"
                                                        class="  m-b-10 m-l-5 "
                                                        id="ajouter">{{ $compromi_enc->getFactureStylimmo()->numero }}
                                            ...
                                            <td>
                                                ...
                                                @if ($compromi_enc->date_vente != null)
                                                    @if (strtotime($compromi_enc->date_vente->format('d-m-Y')) > strtotime(date('d-m-Y')) ||
                                                            $compromi_enc->demande_facture > 0)
                                                        <strong>
                                                            @if ($compromi_enc->date_vente != null)
                                                                {{ $mois[(int) $compromi_enc->date_vente->format('m')] }}-{{ $compromi_enc->date_vente->format('Y') }}
                                                            @endif
                                                        </strong>
                                                    @else
                                                        <strong> <label
                                            ...
                                            <td width="10%" style="{{ $grise }}">
                                                @if ($compromi_enc->charge == 'Vendeur')
                                                    <strong>{{ substr($compromi_enc->nom_vendeur, 0, 20) }}</strong>
                                                @else
                                                    <strong>{{ substr($compromi_enc->nom_acquereur, 0, 20) }}</strong>
                                                @endif
                                            </td>

                                            <td style="{{ $grise }}">
                                                @php
                                                    $com = number_format($compromi_enc->frais_agence() / 1000, '2', '.', ',') . ' K';
                                                @endphp
                                                {{ $com }} €
                                   ...
                                   @if ($compromi_enc->date_mandat != null) {{$compromi_enc->date_mandat->format('d/m/Y')}} @endif  
                                                ...
                                                @else
                                                    @if (Auth()->user()->role == 'admin')
                                                        ...
                                                        @if ($compromi_enc->getPartage() != null)
                                                            <strong> <a
                                                                    href="{{ route('switch_user', Crypt::encrypt($compromi_enc->getPartage()->id)) }}"
                                                                    data-toggle="tooltip"
                                                                    title="@lang('Se connecter en tant que ') {{ $compromi_enc->getPartage()->nom }}">{{ $compromi_enc->getPartage()->nom }}
                                                                    {{ $compromi_enc->getPartage()->prenom }}<i
                                                                        style="font-size: 17px"
                                                                        class="material-icons color-success">person_pin</i></a>
                                                            </strong>
                                                        @else
                                                            <strong> <a data-toggle="tooltip"
                                                                    title="@lang('Agence / Agent externe au réseau STYL\'IMMO') ">{{ $compromi_enc->nom_agent }}
                                                                </a> </strong>
                                                        @endif
                                                    @else
                                                        @if ($compromi_enc->getPartage() != null)
                                                            @if ($compromi_enc->getPartage()->id == Auth()->user()->id)
                                                                <strong> <a>{{ $compromi_enc->user->nom }}
                                                                        {{ $compromi_enc->user->prenom }}</a> </strong>
                                                            @else
                                                                <strong> <a>{{ $compromi_enc->getPartage()->nom }}
                                                                        {{ $compromi_enc->getPartage()->prenom }}</a>
                                                                </strong>
                                                            @endif
                                                        @else
                                                ...
                                                @if ($compromi_enc->je_porte_affaire == 1 && $compromi_enc->agent_id != Auth()->user()->id)
                                                    @if ($compromi_enc->demande_facture == 0)
                                                        ...
                                                        <span><a class="btn btn-default demander_facture"
                                                                href="{{ route('facture.demander_facture', Crypt::encrypt($compromi_enc->id)) }}"
                                                                data-toggle="tooltip"
                                                                date_vente="{{ $compromi_enc->date_vente->format('d/m/Y') }}"
                                                                date-vente="{{ $compromi_enc->date_vente->format('m/d/Y') }}"
                                                    ...
                                                    @else
                                                        <a href="{{ route('facture.telecharger_pdf_facture_stylimmo', Crypt::encrypt($compromi_enc->id)) }}"
                                            ...
                                            <td width="15%">
                                                <a href="{{ route('compromis.show', Crypt::encrypt($compromi_enc->id)) }}"
                                                    data-toggle="tooltip" title="@lang('Détails  ')"><i
                                                        class="large material-icons color-info">visibility</i></a>
                                                @if (
                                                    $compromi_enc->agent_id != Auth()->user()->id &&
                                                        ($compromi_enc->facture_stylimmo_valide == false || Auth()->user()->role == 'admin'))
                                                    <span><a href="{{ route('compromis.show', Crypt::encrypt($compromi_enc->id)) }}"
                                                            data-toggle="tooltip" title="@lang('Modifier ') "><i
                                                                class="large material-icons color-warning">edit</i></a></span>
                                                @endif

                                                @if (
                                                    $compromi_enc->cloture_affaire == 0 &&
                                                        $compromi_enc->demande_facture == 2 &&
                                                        $compromi_enc->agent_id != Auth()->user()->id)
                                                    <a class="cloturer"
                                                        href="{{ route('compromis.cloturer', Crypt::encrypt($compromi_enc->id)) }}"
                                                        data-toggle="tooltip"
                                                        data-mandat="{{ $compromi_enc->numero_mandat }}"
                                                        title="@lang('Réitérer l\'affaire  ')"> <img
                                                            src="{{ asset('images/logo-notaire.png') }}" width="25px"
                                                            height="30px" alt=""> </a>
                                                @elseif($compromi_enc->cloture_affaire == 1)
                                                    @if (Auth()->user()->role != 'admin')
                                                        @if ($compromi_enc->facture_honoraire_partage_cree == 1 || $compromi_enc->facture_honoraire_partage_porteur_cree == 1)
                                                            <a target="blank"
                                                                href="{{ route('facture.preparer_facture_honoraire_partage', Crypt::encrypt($compromi_enc->id)) }}"
                                                                data-toggle="tooltip" title="@lang('Note honoraire  ')"><i
                                                                    class="large material-icons color-danger ">insert_drive_file</i></a>
                                                        @elseif($compromi_enc->facture_honoraire_cree == 1 || $compromi_enc->facture_honoraire_partage_externe_cree == 1)
                                                            <a target="blank"
                                                                href="{{ route('facture.preparer_facture_honoraire', Crypt::encrypt($compromi_enc->id)) }}"
                                                                data-toggle="tooltip" title="@lang('Note honoraire  ')"><i
                                                                    class="large material-icons color-danger">insert_drive_file</i></a>
                                                        @endif
