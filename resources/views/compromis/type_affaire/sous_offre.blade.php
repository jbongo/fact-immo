                            ...
                            <table id="example2"
                                ...
                                <tbody>
                                    @foreach ($compromisSousOffre as $compromi_sous_offre)
                                        <tr>
                                            <td>
                                                @if ($compromi_sous_offre->type_affaire == 'Vente')
                                            ...
                                            @if (Auth()->user()->role == 'admin')
                                                <td>
                                                    <strong> <a
                                                            href="{{ route('switch_user', Crypt::encrypt($compromi_sous_offre->user->id)) }}"
                                                            data-toggle="tooltip"
                                                            title="@lang('Se connecter en tant que ') {{ $compromi_sous_offre->user->nom }}">{{ $compromi_sous_offre->user->nom }}
                                                            {{ $compromi_sous_offre->user->prenom }}<i
                                            ...
                                            <td width="">
                                                @if ($compromi_sous_offre->getFactureStylimmo() != null)
                                                    <a class="color-info" title="Télécharger la facture stylimmo"
                                                        href="{{ route('facture.telecharger_pdf_facture_stylimmo', Crypt::encrypt($compromi_sous_offre->getFactureStylimmo()->id)) }}"
                                                        class="  m-b-10 m-l-5 "
                                                        id="ajouter">{{ $compromi_sous_offre->getFactureStylimmo()->numero }}
                                            ...
                                            <td style="color: #e05555;{{ $grise }}; font-size:18px">
                                                <strong>
                                                    {{ $compromi_sous_offre->created_at->format('d/m/Y') }}</strong>
                                            </td>
                                            <td>
                                                @php
                                                ...
                                                @if ($compromi_sous_offre->date_vente != null)
                                                    @if (strtotime($compromi_sous_offre->date_vente->format('d-m-Y')) > strtotime(date('d-m-Y')) ||
                                                            $compromi_sous_offre->demande_facture > 0)
                                                        <strong>
                                                            @if ($compromi_sous_offre->date_vente != null)
                                                                {{ $mois[(int) $compromi_sous_offre->date_vente->format('m')] }}-{{ $compromi_sous_offre->date_vente->format('Y') }}
                                                            @endif
                                                        </strong>
                                                    @else
                                                        <strong> <label
                                   ...
                                   @if ($compromi_sous_offre->date_mandat != null) {{$compromi_sous_offre->date_mandat->format('d/m/Y')}} @endif  
                                                ...
                                                @else
                                                    @if (Auth()->user()->role == 'admin')
                                                        ...
                                                        @if ($compromi_sous_offre->getPartage() != null)
                                                            <strong> <a
                                                                    href="{{ route('switch_user', Crypt::encrypt($compromi_sous_offre->getPartage()->id)) }}"
                                                                    data-toggle="tooltip"
                                                                    title="@lang('Se connecter en tant que ') {{ $compromi_sous_offre->getPartage()->nom }}">{{ $compromi_sous_offre->getPartage()->nom }}
                                                                    {{ $compromi_sous_offre->getPartage()->prenom }}<i
                                                                        style="font-size: 17px"
                                                                        class="material-icons color-success">person_pin</i></a>
                                                            </strong>
                                                        @else
                                                            <strong> <a data-toggle="tooltip"
                                                                    title="@lang('Agence / Agent externe au réseau STYL\'IMMO') ">{{ $compromi_sous_offre->nom_agent }}
                                                                </a> </strong>
                                                        @endif
                                                    @else
                                                        @if ($compromi_sous_offre->getPartage() != null)
                                                            @if ($compromi_sous_offre->getPartage()->id == Auth()->user()->id)
                                                                <strong> <a>{{ $compromi_sous_offre->user->nom }}
                                                                        {{ $compromi_sous_offre->user->prenom }}</a>
                                                                </strong>
                                                            @else
                                                                <strong> <a>{{ $compromi_sous_offre->getPartage()->nom }}
                                                                        {{ $compromi_sous_offre->getPartage()->prenom }}</a>
                                                                </strong>
                                                            @endif
                                                        @else
                                            ...
                                            <td style="{{ $grise }}">
                                                @if ($compromi_sous_offre->je_porte_affaire == 1 && $compromi_sous_offre->agent_id != Auth()->user()->id)
                                                    @if ($compromi_sous_offre->demande_facture == 0)
                                                        @if ($page_filleul == null)
                                                            ...
                                                            <span><a class="btn btn-default demander_facture"
                                                                    href="{{ route('facture.demander_facture', Crypt::encrypt($compromi_sous_offre->id)) }}"
                                                                    data-toggle="tooltip"
                                                                    date_vente="{{ $compromi_sous_offre->date_vente->format('d/m/Y') }}"
                                                                    date-vente="{{ $compromi_sous_offre->date_vente->format('m/d/Y') }}"
                                                    ...
                                                    @else
                                                        <a href="{{ route('facture.telecharger_pdf_facture_stylimmo', Crypt::encrypt($compromi_sous_offre->getFactureStylimmo()->id)) }}"
                                            ...
                                            <td width="15%">
                                                <a href="{{ route('compromis.show', Crypt::encrypt($compromi_sous_offre->id)) }}"
                                                    data-toggle="tooltip" title="@lang('Détails  ')"><i
                                                        class="large material-icons color-info">visibility</i></a>
                                                @if (
                                                    $compromi_sous_offre->agent_id != Auth()->user()->id &&
                                                        ($compromi_sous_offre->facture_stylimmo_valide == false || Auth()->user()->role == 'admin'))
                                                    @if ($page_filleul == null)
                                                        <span><a href="{{ route('compromis.show', Crypt::encrypt($compromi_sous_offre->id)) }}"
                                                                data-toggle="tooltip" title="@lang('Modifier ') "><i
                                                                    class="large material-icons color-warning">edit</i></a></span>
                                                        {{-- <span><a  href="{{route('compromis.archive',[$compromi_sous_offre->id,1])}}" class="delete" data-toggle="tooltip" title="@lang('Archiver ') {{ $compromi_sous_offre->nom }}"><i class="large material-icons color-danger">thumb_down_alt</i> </a></span> --}}
                                                    @endif
                                                @endif
                                                @if ($page_filleul == null)
                                                    @if (
                                                        $compromi_sous_offre->cloture_affaire == 0 &&
                                                            $compromi_sous_offre->demande_facture == 2 &&
                                                            $compromi_sous_offre->agent_id != Auth()->user()->id)
                                                        <a class="cloturer"
                                                            href="{{ route('compromis.cloturer', Crypt::encrypt($compromi_sous_offre->id)) }}"
                                                            data-toggle="tooltip"
                                                            data-mandat="{{ $compromi_sous_offre->numero_mandat }}"
                                                    ...
                                                    @elseif($compromi_sous_offre->cloture_affaire == 1)
                                                        @if (Auth()->user()->role != 'admin')
                                                            @if (
                                                                $compromi_sous_offre->je_porte_affaire == 0 ||
                                                                    $compromi_sous_offre->agent_id == Auth()->user()->id ||
                                                                    ($compromi_sous_offre->je_porte_affaire == 1 && $compromi_sous_offre->est_partage_agent == 1))
                                                                <a target="blank"
                                                                    href="{{ route('facture.preparer_facture_honoraire_partage', Crypt::encrypt($compromi_sous_offre->id)) }}"
                                                                    data-toggle="tooltip" title="@lang('Note honoraire  ')"><i
                                                                        class="large material-icons color-danger">insert_drive_file</i></a>
                                                            @else
                                                                <a target="blank"
                                                                    href="{{ route('facture.preparer_facture_honoraire', Crypt::encrypt($compromi_sous_offre->id)) }}"
                                                                    data-toggle="tooltip" title="@lang('Note honoraire  ')"><i
                                                                        class="large material-icons color-danger">insert_drive_file</i></a>
                                                            @endif
                                                        @endif
                                                ...
                                                @if (
                                                    ...
                                                    @if ($page_filleul == null)
                                                        ...
                                                        <span><a data-target="#myModal2" data-toggle="modal"
                                                                id="{{ $compromi_sous_offre->id }}" class="archiver"
                                                                href="" data-toggle="tooltip"
                                                                title="@lang('Archiver ') {{ $compromi_sous_offre->nom }}"><i
