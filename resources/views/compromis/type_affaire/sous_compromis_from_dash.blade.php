                            ...
                            <table id="example3"
                                ...
                                <tbody>
                                    @foreach ($compromisSousCompromis as $compromi_sous_compro)
                                        <tr>
                                            <td>
                                                @if ($compromi_sous_compro->type_affaire == 'Vente')
                                            ...
                                            @if (Auth()->user()->role == 'admin')
                                                <td>
                                                    <strong> <a
                                                            href="{{ route('switch_user', Crypt::encrypt($compromi_sous_compro->user->id)) }}"
                                                            data-toggle="tooltip"
                                                            title="@lang('Se connecter en tant que ') {{ $compromi_sous_compro->user->nom }}">{{ $compromi_sous_compro->user->nom }}
                                                            {{ $compromi_sous_compro->user->prenom }}<i
                                            ...
                                            <td width="">
                                                @if ($compromi_sous_compro->getFactureStylimmo() != null)
                                                    <a class="color-info" title="Télécharger la facture stylimmo"
                                                        href="{{ route('facture.telecharger_pdf_facture_stylimmo', Crypt::encrypt($compromi_sous_compro->getFactureStylimmo()->id)) }}"
                                                        class="  m-b-10 m-l-5 "
                                                        id="ajouter">{{ $compromi_sous_compro->getFactureStylimmo()->numero }}
                                            ...
                                            <td>
                                                ...
                                                @if ($compromi_sous_compro->date_vente != null)
                                                    @if (strtotime($compromi_sous_compro->date_vente->format('d-m-Y')) > strtotime(date('d-m-Y')) ||
                                                            $compromi_sous_compro->demande_facture > 0)
                                                        <strong>
                                                            @if ($compromi_sous_compro->date_vente != null)
                                                                {{ $mois[(int) $compromi_sous_compro->date_vente->format('m')] }}-{{ $compromi_sous_compro->date_vente->format('Y') }}
                                                            @endif
                                                        </strong>
                                                    @else
                                                        <strong> <label
                                   ...
                                   @if ($compromi_sous_compro->date_mandat != null) {{$compromi_sous_compro->date_mandat->format('d/m/Y')}} @endif  
                                                ...
                                                @else
                                                    @if (Auth()->user()->role == 'admin')
                                                        ...
                                                        @if ($compromi_sous_compro->getPartage() != null)
                                                            <strong> <a
                                                                    href="{{ route('switch_user', Crypt::encrypt($compromi_sous_compro->getPartage()->id)) }}"
                                                                    data-toggle="tooltip"
                                                                    title="@lang('Se connecter en tant que ') {{ $compromi_sous_compro->getPartage()->nom }}">{{ $compromi_sous_compro->getPartage()->nom }}
                                                                    {{ $compromi_sous_compro->getPartage()->prenom }}<i
                                                                        style="font-size: 17px"
                                                                        class="material-icons color-success">person_pin</i></a>
                                                            </strong>
                                                        @else
                                                            <strong> <a data-toggle="tooltip"
                                                                    title="@lang('Agence / Agent externe au réseau STYL\'IMMO') ">{{ $compromi_sous_compro->nom_agent }}
                                                                </a> </strong>
                                                        @endif
                                                    @else
                                                        @if ($compromi_sous_compro->getPartage() != null)
                                                            @if ($compromi_sous_compro->getPartage()->id == Auth()->user()->id)
                                                                <strong> <a>{{ $compromi_sous_compro->user->nom }}
                                                                        {{ $compromi_sous_compro->user->prenom }}</a>
                                                                </strong>
                                                            @else
                                                                <strong> <a>{{ $compromi_sous_compro->getPartage()->nom }}
                                                                        {{ $compromi_sous_compro->getPartage()->prenom }}</a>
                                                                </strong>
                                                            @endif
                                                        @else
                                                ...
                                                @if ($compromi_sous_compro->je_porte_affaire == 1 && $compromi_sous_compro->agent_id != Auth()->user()->id)
                                                    @if ($compromi_sous_compro->demande_facture == 0)
                                                        ...
                                                        <span><a class="btn btn-default demander_facture"
                                                                href="{{ route('facture.demander_facture', Crypt::encrypt($compromi_sous_compro->id)) }}"
                                                                data-toggle="tooltip"
                                                                date_vente="{{ $compromi_sous_compro->date_vente->format('d/m/Y') }}"
                                                                date-vente="{{ $compromi_sous_compro->date_vente->format('m/d/Y') }}"
                                                    ...
                                                    @else
                                                        <a href="{{ route('facture.telecharger_pdf_facture_stylimmo', Crypt::encrypt($compromi_sous_compro->getFactureStylimmo()->id)) }}"
                                            ...
                                            <td width="15%">
                                                <a href="{{ route('compromis.show', Crypt::encrypt($compromi_sous_compro->id)) }}"
                                                    data-toggle="tooltip" title="@lang('Détails  ')"><i
                                                        class="large material-icons color-info">visibility</i></a>
                                                @if (
                                                    $compromi_sous_compro->agent_id != Auth()->user()->id &&
                                                        ($compromi_sous_compro->facture_stylimmo_valide == false || Auth()->user()->role == 'admin'))
                                                    <span><a href="{{ route('compromis.show', Crypt::encrypt($compromi_sous_compro->id)) }}"
                                                ...
                                                @if (
                                                    $compromi_sous_compro->cloture_affaire == 0 &&
                                                        $compromi_sous_compro->demande_facture == 2 &&
                                                        $compromi_sous_compro->agent_id != Auth()->user()->id)
                                                    <a class="cloturer"
                                                        href="{{ route('compromis.cloturer', Crypt::encrypt($compromi_sous_compro->id)) }}"
                                                        data-toggle="tooltip"
                                                        data-mandat="{{ $compromi_sous_compro->numero_mandat }}"
                                                ...
                                                @elseif($compromi_sous_compro->cloture_affaire == 1)
                                                    @if (Auth()->user()->role != 'admin')
                                                        @if (
                                                            $compromi_sous_compro->je_porte_affaire == 0 ||
                                                                $compromi_sous_compro->agent_id == Auth()->user()->id ||
                                                                ($compromi_sous_compro->je_porte_affaire == 1 && $compromi_sous_compro->est_partage_agent == 1))
                                                            <a target="blank"
                                                                href="{{ route('facture.preparer_facture_honoraire_partage', Crypt::encrypt($compromi_sous_compro->id)) }}"
                                                                data-toggle="tooltip" title="@lang('Note honoraire  ')"><i
                                                                    class="large material-icons color-danger">insert_drive_file</i></a>
                                                        @else
                                                            <a target="blank"
                                                                href="{{ route('facture.preparer_facture_honoraire', Crypt::encrypt($compromi_sous_compro->id)) }}"
                                                                data-toggle="tooltip" title="@lang('Note honoraire  ')"><i
                                                                    class="large material-icons color-danger">insert_drive_file</i></a>
                                                        @endif
                                                    @endif
                                                ...
                                                @if (
                                                    ...
                                                    <span><a data-target="#myModal2" data-toggle="modal"
                                                            id="{{ $compromi_sous_compro->id }}" class="archiver"
                                                            href="" data-toggle="tooltip"
                                                            title="@lang('Archiver ') {{ $compromi_sous_compro->nom }}"><i
