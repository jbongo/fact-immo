    ...
    <div class="col-lg-12">
        ...
        <div class="card alert">
            @if (Auth()->user()->role == 'mandataire')
                <a href="{{ route('compromis.create') }}"
                    class="btn btn-success btn-rounded btn-addon btn-sm m-b-10 m-l-5"><i
                        class="ti-user"></i>@lang('Nouvelle affaire')</a>
                <br><br>
            @endif
            <!-- table -->
                                        ...
                                        <table id="example0" class="display nowrap" style="width:100%">
                                            ...
                                            <tbody>
                                                @foreach ($compromis as $compromi)
                                                    <tr>
                                                        <td>
                                                            @if ($compromi->type_affaire == 'Vente')
                                                        ...
                                                        @if (Auth()->user()->role == 'admin')
                                                            <td>
                                                                <strong> <a
                                                                        href="{{ route('switch_user', Crypt::encrypt($compromi->user->id)) }}"
                                                                        data-toggle="tooltip"
                                                                        title="@lang('Se connecter en tant que ') {{ $compromi->user->nom }}">{{ $compromi->user->nom }}
                                                                        {{ $compromi->user->prenom }}<i
                                                        ...
                                                        <td width="">
                                                            @if ($compromi->getFactureStylimmo() != null)
                                                                <a class="color-info"
                                                                    title="Télécharger la facture stylimmo"
                                                                    href="{{ route('facture.telecharger_pdf_facture_stylimmo', Crypt::encrypt($compromi->getFactureStylimmo()->id)) }}"
                                                                    class="  m-b-10 m-l-5 "
                                                                    id="ajouter">{{ $compromi->getFactureStylimmo()->numero }}
                                                        ...
                                                        <td>
                                                            ...
                                                            @if ($compromi->date_vente != null)
                                                                @if (strtotime($compromi->date_vente->format('d-m-Y')) > strtotime(date('d-m-Y')) || $compromi->demande_facture > 0)
                                                                    <strong>
                                                                        @if ($compromi->date_vente != null)
                                                                            {{ $mois[(int) $compromi->date_vente->format('m')] }}
                                                                            - {{ $compromi->date_vente->format('Y') }}
                                                                        @endif
                                                                    </strong>
                                                                @else
                                                                    <strong> <label
                                                   ...
                                                   @if ($compromi->date_mandat != null) {{$compromi->date_mandat->format('d/m/Y')}} @endif  
                                                            ...
                                                            @else
                                                                @if (Auth()->user()->role == 'admin')
                                                                    ...
                                                                    @if ($compromi->getPartage() != null && $compromi->partage_reseau == 1)
                                                                        <strong> <a
                                                                                href="{{ route('switch_user', Crypt::encrypt($compromi->getPartage()->id)) }}"
                                                                                data-toggle="tooltip"
                                                                                title="@lang('Se connecter en tant que ') {{ $compromi->getPartage()->nom }}">{{ $compromi->getPartage()->nom }}
                                                                                {{ $compromi->getPartage()->prenom }}<i
                                                                                    style="font-size: 17px"
                                                                                    class="material-icons color-success">person_pin</i></a>
                                                                        </strong>
                                                                    @else
                                                                        <strong> <a data-toggle="tooltip"
                                                                                title="@lang('Agence / Agent externe au réseau STYL\'IMMO') ">{{ $compromi->nom_agent }}
                                                                            </a> </strong>
                                                                    @endif
                                                                @else
                                                                    @if ($compromi->getPartage() != null)
                                                                        @if ($compromi->getPartage()->id == Auth()->user()->id)
                                                                            <strong> <a>{{ $compromi->user->nom }}
                                                                                    {{ $compromi->user->prenom }}</a>
                                                                            </strong>
                                                                        @else
                                                                            <strong> <a>{{ $compromi->getPartage()->nom }}
                                                                                    {{ $compromi->getPartage()->prenom }}</a>
                                                                            </strong>
                                                                        @endif
                                                                    @else
                                                                        {{ $compromi->nom_agent }}
                                                            ...
                                                            @if ($compromi->je_porte_affaire == 1 && $compromi->agent_id != Auth()->user()->id)
                                                                @if ($compromi->demande_facture == 0)
                                                                    <span><a class="btn btn-default"
                                                                            href="{{ route('facture.demander_facture', Crypt::encrypt($compromi->id)) }}"
                                                                            data-toggle="tooltip"
                                                                            title="@lang(' ')">demander facture
                                                                            styl</a> </span>
                                                                @elseif($compromi->demande_facture == 1)
                                                                    <span class="color-warning">En attente de
                                                                        validation..</span>
                                                                @else
                                                                    <a href="{{ route('facture.telecharger_pdf_facture_stylimmo', Crypt::encrypt($compromi->getFactureStylimmo()->id)) }}"
                                                        ...
                                                        <td width="15%">
                                                            <a href="{{ route('compromis.show', Crypt::encrypt($compromi->id)) }}"
                                                                data-toggle="tooltip" title="@lang('Détails  ')"><i
                                                                    class="large material-icons color-info">visibility</i></a>
                                                            @if (
                                                                $compromi->agent_id != Auth()->user()->id &&
                                                                    ($compromi->facture_stylimmo_valide == false || Auth()->user()->role == 'admin'))
                                                                <span><a href="{{ route('compromis.show', Crypt::encrypt($compromi->id)) }}"
                                                                        data-toggle="tooltip"
                                                            ...
                                                            @if ($compromi->cloture_affaire == 0 && $compromi->demande_facture == 2 && $compromi->agent_id != Auth()->user()->id)
                                                                <a class="cloturer"
                                                                    href="{{ route('compromis.cloturer', Crypt::encrypt($compromi->id)) }}"
                                                                    data-toggle="tooltip"
                                                                    data-mandat="{{ $compromi->numero_mandat }}"
                                                                    title="@lang('Réitérer l\'affaire  ')"> <img
                                                                        src="{{ asset('images/logo-notaire.png') }}"
                                                                        width="25px" height="30px" alt="">
                                                                    <!-- <i class="large material-icons color-success ">thumb_up_alt</i> -->
                                                                </a>
                                                            @elseif($compromi->cloture_affaire == 1)
                                                                @if (Auth()->user()->role != 'admin')
                                                                    @if ($compromi->facture_honoraire_partage_cree == 1 || $compromi->facture_honoraire_partage_porteur_cree == 1)
                                                                        <a target="blank"
                                                                            href="{{ route('facture.preparer_facture_honoraire_partage', Crypt::encrypt($compromi->id)) }}"
                                                                            data-toggle="tooltip"
                                                                            title="@lang('Note honoraire  ')"><i
                                                                    ...
                                                                    @elseif($compromi->facture_honoraire_cree == 1 || $compromi->facture_honoraire_partage_externe_cree == 1)
                                                                        <a target="blank"
                                                                            href="{{ route('facture.preparer_facture_honoraire', Crypt::encrypt($compromi->id)) }}"
                                                                            data-toggle="tooltip"
                                                                            title="@lang('Note honoraire  ')"><i
                                                                                class="large material-icons color-danger">insert_drive_file</i></a>
                                                                    @else
                                                                        @if (
                                                                            $compromi->je_porte_affaire == 0 ||
                                                                                $compromi->agent_id == Auth()->user()->id ||
                                                                                ($compromi->je_porte_affaire == 1 && $compromi->est_partage_agent == 1))
                                                                            <a target="blank"
                                                                                href="{{ route('facture.preparer_facture_honoraire_partage', Crypt::encrypt($compromi->id)) }}"
                                                                                data-toggle="tooltip"
                                                                                title="@lang('Note honoraire  ')"><i
                                                            ...
                                                            @if (
                                                                ...
                                                                <span><a data-toggle="modal" id="{{ $compromi->id }}"
                                                                        data-target="#myModal2" href=""
                                                                        class="archiver" data-toggle="tooltip"
                                                                        title="@lang('Archiver ') {{ $compromi->nom }}"><i
                            ...
                            <div class="">
                                <div class="form-group row">
                                    <div class="col-lg-8 col-md-8 col-sm-8">
                                        ...
                                        <div>
                                            <input type="radio" id="perte" name="motif_archive"
                                                value="Perte du mandat" checked>
                                            <label for="perte">Perte du mandat</label>
                                        </div>

                                        <div>
                                            <input type="radio" id="retrait" name="motif_archive"
                                                value="Retrait de l'acquéreur ">
                                            <label for="retrait">Retrait de l'acquéreur </label>
                                        </div>

                                        <div>
                                            <input type="radio" id="refus" name="motif_archive"
                                                value="Refus de finacement">
                                            <label for="refus">Refus de finacement</label>
                                        </div>
                                        <div>
                                            <input type="radio" id="deces" name="motif_archive"
                                                value="Décès">
                                            <label for="deces">Décès</label>
                                        </div>
                                        <div>
                                            <input type="radio" id="autre" name="motif_archive"
                                                value="Autre">
    ...
    $(function() {
        ...
        $('body').on('click', 'a.cloturer', function(e) {
            ...
            swalWithBootstrapButtons({
                title: 'Confirmez-vous la réitération de cette affaire (Mandat ' + that.attr(
                    "data-mandat") + ' )  ?',
                type: 'warning',
    ...
    $('.demander_facture').click(function(e) {
        ...
        if (diffDays > nb_jour_max_demande && diffDays > 0) {
            ...
            swal(
                ...
                'La demande de facture ne peut être faite plus de ' + nb_jour_max_demande +
                ' jours avant la date de vente: ( ' + $(this).attr('date_vente') + ' )',
                'error'
            )
            // swal(diffDays +'jours')
