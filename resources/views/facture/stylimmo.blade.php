                            ...
                            <table  id="example1" class=" table student-data-table  table-striped table-hover dt-responsive display    "  style="width:100%"  >
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td width=""  >
                                            <label data-toggle="tooltip" title="Facture pub réglée par le mandataire"  style="color:rgb(255, 4, 0);">Réglée le {{string_to_date( $facture->date_reglement)}} </label>
                                        </td>
                                    ...
                                    {{-- fin alert paiement --}}
                                        ...
                                        <td width="" >
                                            ...
                                            @if($facture->type == "avoir")
                                                <label class="color-danger"> Avoir sur {{get_facture_avoir($facture->facture_id)->numero}}</label> 
                                            @else
                                                {{-- Si la facture stylimmo a un avoir --}}
                                                @if($facture->a_avoir == 1 && get_avoir_facture($facture->id)  != null)
                                                    <label class="color-primary"> annulée par AV {{get_avoir_facture($facture->id) ->numero}}</label> 
                                                ...
                                                @else
                                                    ...
                                                    @else 
                                                    <label class="color-danger"> @if($facture->date_encaissement != null) encaissée le {{string_to_date( $facture->date_encaissement)}} @else encaissée @endif  </label> 
                                                    @endif 
                                                @endif
                                        ...
                                        <td width="" >
                                            ...
                                            @if($facture->type != "avoir")
                                                ...
                                                @elseif($facture->a_avoir == 0 && $facture->encaissee == 0 && $facture->compromis == null  && auth()->user()->role == "admin") 
                                                    ...
                                                    <a href="{{route('facture.avoir.create', Crypt::encrypt($facture->id))}}" target="_blank"  class="btn btn-info  btn-flat btn-addon  m-b-10 m-l-5 " id=""><i class="ti-link"></i>créer</a>
                                                @elseif($facture->a_avoir == 1 && get_avoir_facture($facture->id) != null)
                                                    <a href="{{route('facture.telecharger_pdf_avoir', Crypt::encrypt(get_avoir_facture($facture->id)->id))}}"  class="btn btn-danger btn-flat btn-addon m-b-10 m-l-5 " id=""><i class="ti-download"></i>avoir {{get_avoir_facture($facture->id)->numero}}</a>
                                                @endif
                                            @endif
                                        </td>
                                    </tr> 
