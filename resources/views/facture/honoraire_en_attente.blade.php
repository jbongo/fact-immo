 ...
 @section('content')
     @section ('page_title')
     <a href="{{route('home')}}" class="btn btn-warning btn-flat btn-addon m-b-10 m-l-5"><i class="ti-angle-double-left"></i>@lang('Retour')</a> 
        <hr>
     Affaires en attente d'encaissement en  {{$annee}} <br><hr>
                    ...
                    <div class="container-fluid">
                      ...
                      <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
                        <ul class="nav navbar-nav">
                          <li class="active"><a href="{{route('facture.index_honoraire')}}"> <i class="material-icons" style="font-size: 15px;">account_balance_wallet</i> Factures Honoraires en attente d'encaissement <span class="sr-only">(current)</span></a></li>
                                            ...
                                            @else
                                                ...
                                                @elseif($facture->type == "parrainage" || $facture->type == "parrainage_partage")
                                                    @if($facture->filleul_id != null)
                                                        <a target="blank" href="{{route('facture.preparer_facture_honoraire_parrainage',[Crypt::encrypt($facture->compromis->id), $facture->filleul_id])}}" data-toggle="tooltip" title="@lang('Note honoraire parrainage')"><i class="large material-icons color-danger">insert_drive_file</i></a> 
                                                    @else 
                                                        <a target="blank" href="{{route('facture.preparer_facture_honoraire_parrainage',[Crypt::encrypt($facture->compromis->id), $facture->user_id])}}" data-toggle="tooltip" title="@lang('Note honoraire parrainage')"><i class="large material-icons color-danger">insert_drive_file</i></a> 
                                                    @endif 
...
<script>
    ...
    var annee = $('#annee').val();
      window.location.href = "/factures/honoraire_encaissee/"+annee;
