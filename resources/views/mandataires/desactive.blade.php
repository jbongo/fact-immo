                ...
                <table id="example3"
                    ...
                    <tbody>
                        @foreach ($mandataires as $mandataire)
                            ...
                            @if (
                                $mandataire->contrat != null &&
                                    $mandataire->contrat->a_demission == true &&
                                    $mandataire->contrat->est_fin_droit_suite == true)
                                <tr>

                                    <td>
                                        <a href="{{ route('switch_user', Crypt::encrypt($mandataire->id)) }}"
                                            data-toggle="tooltip"
                                            title="@lang('Se connecter en tant que ') {{ $mandataire->nom }}">{{ $mandataire->nom }}
                                            {{ $mandataire->prenom }}<i style="font-size: 17px"
                                    ...
                                    <td style="color: #32ade1;  ">
                                        @if ($mandataire->contrat != null)
                                            @if ($mandataire->contrat->deduis_jeton == true)
                                                <a href="{{ route('mandataire.historique_jeton', Crypt::encrypt($mandataire->id)) }}"
                                                    class="badge badge-default"><i style="font-size: 15px"
                                                        class="material-icons color-success ">launch</i> <span
                                                        style="font-size: 22px"> {{ $mandataire->nb_mois_pub_restant }}
                                                    </span></a>
                                            @else
                                    ...
                                    <td
                                        @if ($mandataire->contrat == null) style="background:#757575; color:white" @endif>
                                        <span class="color-success">
                                            @if ($mandataire->contrat != null)
                                                {{ $mandataire->commission }} %
                                            @else
                                                Pas de contrat
                                            @endif
                                        </span>
                                    ...
                                    <td width="13%">
                                        <span><a href="{{ route('mandataire.show', Crypt::encrypt($mandataire->id)) }}"
                                                data-toggle="tooltip"
                                                title="@lang('Détails de ') {{ $mandataire->nom }}"><i
                                                    class="large material-icons color-info">visibility</i></a> </span>
                                        <span><a href="{{ route('mandataire.edit', Crypt::encrypt($mandataire->id)) }}"
                                                data-toggle="tooltip"
                                                title="@lang('Modifier ') {{ $mandataire->nom }}"><i
                                        ...
                                        <span><a href="{{ route('mandataire.activer', [$mandataire->id]) }}"
                                                class="activer"
                                                contrat="{{ route('contrat.edit', Crypt::encrypt($mandataire->contrat->id)) }}"
                                                data-toggle="tooltip"
                                                title="@lang('Activer ') {{ $mandataire->nom }}"><i
                                                    class="large material-icons color-danger">replay</i> </a></span>
                                    </td>
                                </tr>
                            @endif
