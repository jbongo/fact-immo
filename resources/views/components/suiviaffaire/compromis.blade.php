@php
    ...
    if (unserialize($bien->documents_mandant) != false) {
        $data = unserialize($bien->documents_mandant);
        foreach ($data as $one) {
            if ($one['compromis'] === 1) {
                $all += 1;
    ...
    if (unserialize($bien->documents_acquereur) != false) {
        $data = unserialize($bien->documents_acquereur);
        foreach ($data as $one) {
            if ($one['compromis'] === 1) {
                $all += 1;
                if ($one['statut'] === 'Disponible') {
...
@endphp
<div class="modal fade" id="gestion_compromis" data-backdrop="static" tabindex="-1" role="dialog"
        ...
        <div class="modal-content">
            ...
            <div class="modal-body tet18">
                <ul class="nav nav-pills nav-tabs" id="zobi">
                    <li class="cpr111 active"><a data-toggle="pill" href="#home_cmpr"><i class="ti-wallet"></i>
                            <strong>@lang('Liste des documents')</strong></a></li>
                    @if ($bien->compromis === 1)
                        <li class="cpr222"><a data-toggle="pill" href="#nrvcmpr1"><i class="ti-key"></i>
                                <strong>@lang('Conditions suspensives')</strong></a></li>
                    @endif
                    <li class="cpr333"><a data-toggle="pill" href="#nrvcmpr2"><i class="ti-files"></i>
                            <strong>@lang('Détails du rendez-vous')</strong></a></li>
                </ul>
                <br><br>
                        ...
                        @if ($bien->notaires === null)
                            <div class="panel lobipanel-basic panel-danger">
                                ...
                                <div class="panel-body">
                                    Vous devez définir les notaires pour le compromis et l'acte au préalable avant de
                                    pouvoire fixer le rendez-vous pour la signature du compromis.
                                </div>
                            </div>
                        @else
                            @if ($bien->rendez_vous_compromis === null)
    ...
    @if ($bien->compromis === 1)
        <script>
                ...
                $('.submitcondition').on('click', function(f) {
                    ...
                    if (form.valid() === true) {
                        $.ajax({
                            type: "POST",
                            url: (
                                "{{ route('suiviaffaire.offre.condition.add', $compromis_actif != null ? Crypt::encrypt($compromis_actif->id) : 'undefined') }}"
                                ),
            ...
            $('a.reject_condition').on('click', function(b) {
                ...
                var route = that.attr('href');
                var warning =
                    'Valider uniquement si la condition suspensive ne peut pas étre accomplie et ainsi annuler le compromis, continuer ?';
                var div = ".lst_conditions";
                processAjaxSwalDivRefresh(route, warning, div);
            });
        </script>
    ...
    <script>
        ...
        $("#submit_compromis").on('click', function(rtf) {
            ...
            if (form.valid() == true) {
                ...
                $.ajax({
                    url: (
                        "{{ route('suiviaffaire.rdv', [Crypt::encrypt($bien->id), Crypt::encrypt('rendez_vous_compromis')]) }}"
                        ),
                    type: 'POST',
                    data: {
        ...
        <script>
            ...
            $(".update_compromis").on('click', function(orion) {
                ...
                if (form.valid() === true) {
                    $.ajax({
                        url: (
                            "{{ route('suiviaffaire.rdv.update', [Crypt::encrypt($bien->id), Crypt::encrypt('rendez_vous_compromis')]) }}"
                            ),
                        type: 'POST',
                        data: {
                        ...
                        success: function(data) {
                            var ret = $("#global_infos_compromis").load(location.href +
                                " #global_infos_compromis");
                            $('.pace').addClass('pace-inactive');
                            swal(
                                'Effectué',
                                'Le rendez-vous a été plannifié !',
