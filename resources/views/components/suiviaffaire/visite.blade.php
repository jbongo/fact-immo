@if ($bien->statut === 'offre' || $bien->statut === 'debut')
            ...
            @foreach ($bien->visites as $one)
                <tr>
                    @if ($one->visitable_type === 'App\Models\Entite')
                        @php $route = route('contact.entite.show', Crypt::encrypt($one->visitable_id)) @endphp
                    @else
                        @php $route = route('contact.individu.show', Crypt::encrypt($one->visitable_id)) @endphp
                    @endif
                    <td>
                    ...
                    <td>
                        <span><a href="{{ $route }}" data-toggle="tooltip" title="Voir le visiteur"><i
                                    class="large material-icons color-info">visibility</i></a> </span>
                    </td>
                    <td>
                        <span><a href="{{ route('suiviaffaire.visite.download', Crypt::encrypt($one->id)) }}"
                                data-toggle="tooltip" title="Télécharger le bon de visite"><i
                                    class="large material-icons color-pink">cloud_download</i></a> </span>
                    </td>
                    <td>
                        @if ($one->etat === 'Préparation')
                            <span><a class="confirm_visite"
                                    href="{{ route('suiviaffaire.visite.confirm', [Crypt::encrypt($one->id), 'Effectuée']) }}"
                                    data-toggle="tooltip" title="Confirmer la visite"><i
                                        class="large material-icons color-success">check</i></a> </span>
                            <span><a class="cancel_visite"
                                    href="{{ route('suiviaffaire.visite.confirm', [Crypt::encrypt($one->id), 'Annulée']) }}"
                                    data-toggle="tooltip" title="Annuler la visite"><i
                                        class="large material-icons color-warning">close</i></a> </span>
                            <span><a class="delete_visite"
                                    href="{{ route('suiviaffaire.visite.delete', Crypt::encrypt($one->id)) }}"
                                    data-toggle="tooltip" title="Supprimer la visite"><i
                                        class="large material-icons color-danger">delete</i></a> </span>
                        @else
                            <span><a href="#" data-toggle="tooltip" title="Voir le compte rendu"
                                    onclick="Swal.fire({html: '<p>{{ $one->compte_rendu }}</p>'})"><i
                                        class="large material-icons color-dark">speaker_notes</i></a> </span>
                        @endif
                    </td>
                </tr>
            @endforeach
