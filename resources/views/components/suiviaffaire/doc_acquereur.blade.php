<a href="#" data-toggle="modal" data-target="#documents_acquereur_add"
            ...
            @if (unserialize($bien->documents_acquereur) != false)
                @foreach (unserialize($bien->documents_acquereur) as $key => $one)
                    <tr>
                        ...
                        <td>
                            <input type="checkbox"
                                route="{{ route('suiviaffaire.document.action', [Crypt::encrypt($bien->id), Crypt::encrypt($key), 'documents_acquereur', 'toggle_compromis']) }}"
                        ...
                        <td>
                            <input type="checkbox"
                                route="{{ route('suiviaffaire.document.action', [Crypt::encrypt($bien->id), Crypt::encrypt($key), 'documents_acquereur', 'toggle_acte']) }}"
                        ...
                        <td>
                            @if ($one['statut'] === 'Traitement')
                                <span><a class="validate_document"
                                        href="{{ route('suiviaffaire.document.action', [Crypt::encrypt($bien->id), Crypt::encrypt($key), 'documents_acquereur', 'confirm']) }}"
                            ...
                            <span><a class="delete_document"
                                    href="{{ route('suiviaffaire.document.action', [Crypt::encrypt($bien->id), Crypt::encrypt($key), 'documents_acquereur', 'delete']) }}"
