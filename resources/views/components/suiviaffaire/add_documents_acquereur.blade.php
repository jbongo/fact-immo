<div class="modal fade" id="documents_acquereur_add" data-backdrop="static" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
          ...
          <div class="modal-body">
                <div class="panel lobipanel-basic panel-dark">
                        ...
                        <div class="panel-body">
                           Définnir la liste des documents justificatifs que l'acquéreur' doit réunir pour la signature du compromis et de l'acte en fonction de sa situation, cette liste lui
                           sera envoyée par email vous pouvez valider en suite le document dès la confirmation sa disponibilité par l'acquéreur ou le cas echean, sa réception, vous pouvez ajouter jusqu'à 10 documents à la fois.
                        </div>
                     </div>
             <div class="form-validation">
                <form class="form-appel form-horizontal doc_acq_push" action="#" method="post">
                 @csrf
                 <button id="add_field_documents_acquereur" class="btn btn-pink btn-rounded btn-addon btn-sm m-b-10 m-l-5"><i class="ti-plus"></i>Ajouter une ligne</button>
...
<script>
 ...
 if (form.valid() === true){
   ...
   $('#documents_acquereur_add').modal('toggle');
     $.ajax({
         type: "POST",
         url: ("{{route('suiviaffaire.document.add', ['documents_acquereur', CryptId($affaire->id)])}}"),
