<button class="btn btn-success btn-rounded btn-addon btn-sm m-b-10 m-l-5" id="show_form_compromis"><i
        class="ti-plus"></i>@lang('Ajouter')</button>
<!--form conditions-->
<div class="form-validation form_conditions_suspensives" hidden="true">
    <form class="form-horizontal form-condition_compromis" action="#" method="post">
        ...
        <div class="form-group row">
            <label class="col-sm-4 control-label" for="intitule">Intitulé de la condition suspensive<span
        ...
        <div class="form-group row">
            ...
            <div class="col-lg-3">
                <input type="date" min="" id="fin_condition" class="form-control" value=""
                    name="fin_condition" required>
            </div>
        </div>
        <div class="form-group row" style="text-align: center;">
            <button type="submit" id="compromis_check"
                class="btn btn-primary submitcondition"><strong>Valider</strong></button>
        </div>
    </form>
</div>
<!--end form-->
            ...
            @if ($compromis_actif != null)
                @foreach ($compromis_actif->conditionoffres as $one)
                    <tr>
                        ...
                        <td>
                            @if ($one->statut === 'En cours')
                                <span><a class="confirm_condition"
                                        href="{{ route('suiviaffaire.offre.condition.action', [Crypt::encrypt($one->id), 'Accomplie']) }}"
                                        data-toggle="tooltip" title="Valider la condition"><i
                                            class="large material-icons color-success">check</i></a> </span>
                                <span><a class="reject_condition"
                                        href="{{ route('suiviaffaire.offre.condition.action', [Crypt::encrypt($one->id), 'Non accomplie']) }}"
                                        data-toggle="tooltip" title="Ajourner la condition"><i
                                            class="large material-icons color-danger">close</i></a> </span>
                            @endif
                        </td>
                    </tr>
                @endforeach
