<div class="modal fade" id="mandat_add" data-backdrop="static" tabindex="-1" role="dialog"
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="form-validation">
                    <form class="form-appel form-horizontal form-valid-mandat" action="{{ route('mandat.store') }}"
                        method="post">
                        @csrf
                        <div class="form-group row">
                            <label class="col-sm-4 control-label" for="objet">Objet du mandat<span
                        ...
                        <div class="form-group row">
                            <label class="col-sm-4 control-label" for="type">Type du mandat<span
                                    class="text-danger">*</span></label>
                            <div class="col-lg-3">
                        ...
                        <div class="form-group row" id="op1">
                            <label class="col-sm-4 control-label" for="bien_id">Associer le bien<span
                                    class="text-danger">*</span></label>
                            <div class="col-lg-4">
                                <select class="selectpicker col-lg-8" id="bien_id" name="bien_id"
                                    ...
                                    @foreach ($biens as $bien)
                                        @foreach ($bien->photosbiens as $photos)
                                            @if ($photos->visibilite == 'visible' && $photos->image_position == 1)
                                                @php $src=asset("images/photos_bien/".$photos->filename)@endphp @php $offset = 1 @endphp
                                            @break
                                        @endif
                                        @endforeach @if ($offset == null)
                                            @php $src=asset('/images/common/'."home.jpg")@endphp
                                        @else
                                            @php $offset = NULL @endphp
                                        @endif
                                        <option value="{{ Crypt::encrypt($bien->id) }}"
                                            data-content="<img class='square-img' src='{{ $src }}' alt=''> <strong>__{{ $bien->titre_annonce }}</strong>"
                                            data-tokens="{{ $bien->titre_annonce }}"></option>
                                    @endforeach
                            </select>
                        </div>
                    </div>
                    <div class="form-group row" id="fcf55">
                        <label class="col-sm-4 control-label" for="entite_id">Associer l'entité<span
                                class="text-danger">*</span></label>
                        <div class="col-lg-4">
                    ...
                    <div class="form-group row @if ($errors->has('numero')) has-error @endif">
                        <label class="col-sm-4 control-label" for="numero">Numéro du mandat<span
                                class="text-danger">*</span></label>
                        <div class="col-lg-3">
                            <input type="text" id="numero"
                                class="form-control {{ $errors->has('numero') ? 'is-invalid' : '' }}"
                                value="{{ old('numero') }}" name="numero" placeholder="Ex: 99258... " required>
                        </div>
                    </div>
                    <div class="form-group row @if ($errors->has('date_debut')) has-error @endif">
                        <label class="col-sm-4 control-label" for="date_debut">Date du début <span
                                class="text-danger">*</span></label>
                        <div class="col-lg-3">
                    ...
                    <div class="form-group row @if ($errors->has('date_fin')) has-error @endif">
                        <label class="col-sm-4 control-label" for="date_fin">Date de fin <span
                                class="text-danger">*</span></label>
                        <div class="col-lg-3">
                            <input type="date" id="date_fin"
                                class="form-control {{ $errors->has('date_fin') ? 'is-invalid' : '' }}"
                                value="{{ old('date_fin') }}" name="date_fin" required>
                        </div>
                    </div>
                    <div class="form-group row @if ($errors->has('describe')) has-error @endif">
                        ...
                        <div class="col-lg-3">
                            <textarea rows="4" id="describe" class="form-control {{ $errors->has('describe') ? 'is-invalid' : '' }}"
                                value="{{ old('describe') }}" name="describe" placeholder="..."></textarea>
                        </div>
                    </div>
