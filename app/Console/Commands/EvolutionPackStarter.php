...
class EvolutionPackStarter extends Command
{
    ...
    public function handle()
    {
        ...
        foreach ($mandataires as $mandataire) {
            ...
            if($contrat != null && $contrat->a_demission == false){
           
                $passage_expert = false;
                $nb_mois_starter = $contrat->duree_max_starter;
           
                // le parametre nb_vente est prioritaire pour le passage à expert s'il est renseigné dans le contrat du mandataire
                if($contrat->nb_vente_passage_expert > 0){

                    // Si le mandataire dépasse le nombre vente requis, on le passe expert  
                    $date_deb = $contrat->date_deb_activite->format('Y-m-d').
                    $date_fin = date('Y-m-d');
                    $passage_expert =  $mandataire->nombre_vente($date_deb, $date_fin) >= $contrat->nb_vente_passage_expert   ? true : false ;
                    ...
                    if(date('Y-m-d',strtotime($contrat->date_deb_activite->format('Y-m-d')."+ $nb_mois_starter month")) <= date('Y-m-d') ){
                        $passage_expert = true;
                    }
                ...
                if($passage_expert == true){
                    ...
                    $nouvelle_comm = $contrat->pourcentage_depart_expert;
                    $mandataire->commission = $nouvelle_comm;
                    
                    $contrat->date_passage_starter_expert = date("Y-m-d");
                    
                    $contrat->update();
                    
                    $mandataire->update();
    
                    Mail::to($mandataire->email)->send(new NotifEvolutionStarter($mandataire,$nb_mois_starter,$contrat->date_deb_activite,$ancienne_comm,$nouvelle_comm));
                    Mail::to("support@stylimmo.com")->send(new NotifEvolutionStarter($mandataire,$nb_mois_starter,$contrat->date_deb_activite,$ancienne_comm,$nouvelle_comm));
                }
